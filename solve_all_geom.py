...
def bfs(target_edges, layout_id, is_advanced, init_seq):
    init_seq_norm = normalize_simple(init_seq)
    
    if get_edge_set(init_seq_norm, layout_id) == target_edges: return 0
    
    q_f = deque([(init_seq_norm, 0)])
    visited_f = {init_seq_norm: 0}
    
    while q_f:
        curr_f, d_f = q_f.popleft()
        
        if d_f >= 10: # Stop search at depth 10
            continue
            
        for nxt in get_neighbors(curr_f, is_advanced):
            if nxt not in visited_f:
                visited_f[nxt] = d_f + 1
                if get_edge_set(nxt, layout_id) == target_edges:
                    return d_f + 1
                q_f.append((nxt, d_f + 1))
...
for lvl in levels:
    ...
    ans = bfs(target_edges, lvl["layout_id"], lvl["is_adv"], init_seq)
