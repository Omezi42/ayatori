...
def bfs(target, is_advanced):
    ...
    q_f = deque([(init_seq, 0)])
    q_b = deque([(target_seq, 0)])
    
    visited_f = {init_seq: 0}
    visited_b = {target_seq: 0}
    
    while q_f and q_b:
        curr_f, d_f = q_f[0]
        curr_b, d_b = q_b[0]
        
        if d_f + d_b > 9: # safety limit
            return -1
